...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      // Obtener photocards del usuario
      let query = supabase
        .from('user_cards')
        .select(`
          id,
          ...
          .setDescription(
            shown
              .map(c => {
                ...
                return `${rarity.stars} ${c.base_cards.name} — ${c.base_cards.group_name || 'sin grupo'} (${c.base_cards.era || 'desconocida'})\n${c.unique_card_id || c.base_cards.card_code || c.id}`;
              })
              .join('\n\n')
          )
      ...
      collector.on('collect', async i => {
        if (i.customId === 'prev') {
          ...
          await i.reply({
            content: '🆔 **Códigos de esta página:**\n' +
              shown.map(c => `\`${c.unique_card_id || c.base_cards.card_code || c.id}\` → ${c.base_cards.name}`).join('\n'),
            ephemeral: true
          });
          ...
          if (packsError) {
            ...
            return i.reply({ content: '❌ Error al cargar tus packs.', ephemeral: true });
          }
          ...
          await i.reply({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(list)
                .setFooter({ text: `Total: ${packs.reduce((acc, p) => acc + p.quantity, 0)} packs` })
                .setTimestamp()
            ],
            ephemeral: true
          });
          ...
          await i.deleteReply().catch(() => {}); // Mensaje desaparece al cerrar
        }
      });
