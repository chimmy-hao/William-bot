...
const rarityConfig = {
  1: { 
    display: `${strawberryEmoji}`, // 1 Fresa
    name: 'Rareza 1', 
    color: '#95a5a6' 
  },
  2: { 
    display: `${strawberryEmoji}${strawberryEmoji}`, // 2 Fresas
    name: 'Rareza 2', 
    color: '#3498db' 
  },
  3: { 
    display: `${strawberryEmoji}${strawberryEmoji}${strawberryEmoji}`, // 3 Fresas
    name: 'Rareza 3', 
    color: '#9b59b6' 
  }
};
...
// CAMBIO: 5 minutos en milisegundos
  ...
  async execute(interaction) {
    ...
    if (remaining > 0) {
      ...
      return interaction.reply({
        content: `⏳ Debes esperar **${minutes}m ${seconds}s** antes de volver a tirar.`,
        ephemeral: true
      });
    }
    ...
    try {
      ...
      // Determinar nivel numérico de rareza (fallback si rarity_level es null)
      ...
      // Formateo de texto: Nombre — Grupo
      const groupText = randomCard.group_name ? ` — ${randomCard.group_name}` : '';
      const eraText = randomCard.era ? `\nEra: ${randomCard.era}` : '';

      const embed = new EmbedBuilder()
        .setColor(rConfig.color)
        .setTitle('✨ ¡Nueva Photocard Obtenida! ✨')
        .setDescription(`*${randomCard.name}*${groupText}${eraText}`)
        .addFields(
          { name: '🎴 ID de Carta', value: `\`${uniqueId}\``, inline: true },
          // Aquí usamos el emoji personalizado repetido según la rareza
      ...
      await interaction.editReply('❌ Hubo un error al procesar tu drop. Intenta de nuevo.');
    }
  }
};